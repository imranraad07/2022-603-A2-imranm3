...
__global__ void KNN(float * test, int * predictions, int train_size, int test_size, int k, int num_attributes, int num_classes, int stream, int numberElementsPerStream) {
...
int main(int argc, char * argv[]) {
    ...
    cudaMemcpy(&d_test_instances[test_size * num_attributes], &h_test_instances[test_size * num_attributes], train_size * num_attributes * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (int i = 0; i < numStreams; i++)
    {
        ...
        KNN <<< blocksPerGrid, THREADS_PER_BLOCK, 0, streams[i] >>> (d_test_instances, d_predictions, train_size, test_size, k, num_attributes, num_classes, i, numberElementsPerStream);
